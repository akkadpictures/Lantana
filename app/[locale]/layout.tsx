...
import { lantanaMetadata } from "@/lib/seoLantana";
...
import LantanaJsonLd from "@/components/LantanaJsonLd";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return { ...baseMetadata(loc), ...lantanaMetadata(loc) };
}
  ...
  return (
    <html lang={locale} dir={dir(locale)} className={`${display.variable} ${body.variable} ${displayAr.variable} ${bodyAr.variable}`}>
      <body className="font-body">
        <LantanaJsonLd locale={locale} />
        <MarketProvider market={market} locale={locale}>
          <Preloader />
          ...
          <RegionBar locale={locale} dict={dict} />
