    ...
    <div className="flex min-h-screen">
      ...
      <aside className={cn("fixed inset-y-0 left-0 z-40 w-60 -translate-x-full border-r hairline bg-ivory-soft transition-transform duration-300 lg:static lg:translate-x-0", open && "translate-x-0")}>
        <div className="flex items-center gap-2 border-b hairline px-6 py-5">
          ...
          <span className="font-display text-lead tracking-wide2">LANTANA</span>
        </div>
          ...
          {NAV.map((n) => {
            ...
            return (
              <Link key={n.href} href={n.href} onClick={() => setOpen(false)}
                className={cn("rounded px-3 py-2 font-body text-base2 transition-colors", active ? "bg-olive text-ivory" : "text-ink/70 hover:bg-ink/5")}>
                {n.label}
              </Link>
            );
          })}
        </nav>
        <button onClick={logout} className="mx-3 mt-2 rounded px-3 py-2 text-start font-body text-base2 text-ink/60 hover:bg-ink/5">Sign out</button>
      </aside>
      ...
      <div className="flex-1">
        <header className="sticky top-0 z-20 flex items-center justify-between border-b hairline bg-ivory/90 px-5 py-4 backdrop-blur md:px-8">
          ...
          <h1 className="font-display text-d4 font-light">{title}</h1>
          <Link href="/en" target="_blank" className="font-body text-micro uppercase tracking-wide2 text-ink/50 hover:text-ink">View site ↗</Link>
        </header>
